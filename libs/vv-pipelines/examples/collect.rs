...
fn main() -> anyhow::Result<()> {
    ...
    let broadcast1 = graph.add_component((BroadcastVec::<Vec<i32>>::new(), "broadcast1"))?;
    let broadcast2 = graph.add_component((BroadcastVec::<i32>::new(), "broadcast2"))?;

    let (collect1, collect2);
    if mock {
        use mocks::*;
        collect1 = graph.add_component((CollectVecComponent::<i32>::new(), "collect1"))?;
        collect2 = graph.add_component((CollectVecComponent::<i32>::new(), "collect2"))?;
    } else {
        use vv_pipelines::components::prelude::*;
        collect1 = graph.add_component((CollectVecComponent::<i32>::new(), "collect1"))?;
        collect2 = graph.add_component((CollectVecComponent::<i32>::new(), "collect2"))?;
    };
    let print1s = graph.add_component((Print, "print-1-sorted"))?;
    let print2s = graph.add_component((Print, "print-2-sorted"))?;
    let print1u = graph.add_component((Print, "print-1-unsorted"))?;
    let print2u = graph.add_component((Print, "print-2-unsorted"))?;
