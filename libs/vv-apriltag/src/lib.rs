...
impl vv_vision::draw::Drawable for Detection {
    fn draw(&self, color: &[u8], buffer: &mut Buffer) {
        ...
        for i in 0..4 {
            ...
            let Ok(it) =
                vv_vision::draw::DrawLineIterator::new(x0 as i32, y0 as i32, x1 as i32, y1 as i32);
            for (x, y) in it {
...
pub struct PoseParams {
    ...
    #[cfg_attr(feature = "serde", serde(deserialize_with = "tag_size::deserialize"))]
    pub tag_size: f64,
